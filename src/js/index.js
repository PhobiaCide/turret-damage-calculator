...
class TurretMatrix {
  ...
  calculate() {
    ...
    const angleRad = degreesToRadians(
      parseFloat(inputs.targetAngle.value) || 0
    );
    const velocity = parseFloat(inputs.targetVelocity.value) || 0;

    const maxRange = optimal + 2 * falloff;
    this.array = Array.from(
      { length: Math.ceil((maxRange - 250) / 250) },
      (_, i) => {
        const range = 250 + 250 * i;

        const transversal = velocity * Math.sin(angleRad);
        const trackingTerm = Math.pow(
          (transversal / (range * tracking)) * (targetSig / turretSig),
          2
        );
        const falloffTerm = Math.pow(
          Math.max(0, (range - optimal) / falloff),
          2
        );
        ...
        annotation: {
          annotations: {
            ...
            falloff1: {
              type: "box",
              xMin: optimal / 300,
              xMax: optimal / 264 + falloff / 264,
              backgroundColor: "rgba(255, 255, 100, 0.1)",
              label: {
                enabled: true,
                content: "Optimal + Falloff",
                position: "start",
              },
            },
            falloff2: {
              type: "box",
              xMin: optimal / 264 + falloff / 264,
              xMax: optimal / 250 + (2 * falloff) / 250,
              backgroundColor: "rgba(255, 100, 100, 0.1)",
              label: {
                enabled: true,
                content: "Optimal + 2x Falloff",
                position: "start",
              },
            },
          },
